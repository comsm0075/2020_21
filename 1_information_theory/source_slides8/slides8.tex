...
\begin{frame}{Continuous to discrete}
\begin{tikzpicture}
\begin{axis}[
...
%\draw [yshift=-0.6cm, latex-latex](axis cs:4,0) -- node [fill=white] {$1.96\sigma$} (axis cs:5.96,0);
\end{axis}
  \draw (5.5,2) node{\color{cyan!50!black}$p_X(x)$\cbla};
\end{tikzpicture}
\end{frame}
...
\end{axis}
  \draw (3.1,-0.3) node{$x_i$};
  \draw (3.8,-0.31) node{$x_{i+1}$};
  \draw (1.7,2.6) node{\color{cyan}$q_i=\int_{x_{i-1}}^{x_i}dx\,p_x(x)$\cbla};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{Continuous to discrete}
  \crish $$
  q_i = \int_{x_{i-1}}^{x_i}dx\,p_x(x)
  $$\cbla
  The plot obviously is to get rid of the integral here, morally:
  \crish $$
    q_i=\int_{x_{i}}^{x_{i+1}}dx,p_X(x)\approx p(\bar{x}_i)\delta 
$$\cbla
